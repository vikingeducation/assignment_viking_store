...
def create_users(start_date, end_date)
  User.create(first_name: Faker::Name.first_name,
              ...
              created_at: Faker::Date.between(start_date, end_date))
end

def create_address
  Address.create(street: Faker::Address.street_address,
                city: CITY_NAME.sample,
                state: STATE_NAME.sample,
...
def create_billing(user_num, address_num)
  Billing.create(user_id: user_num,
...
def user_make_order(userid)
  user_order = Order.create(user_id: userid,
                checkout: Faker::Date.between(1.years.ago, Time.now))

  PackingList.create(product_id: products.sample.id,
                    order_id: user_order.id, quantity: rand(1..10))
end
