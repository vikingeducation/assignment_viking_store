class Billing < ActiveRecord::Base
  ...
  def self.top_3_cities(index)
      @city_name = Billing.select("city, count(user_id) AS usercount").group(:city).order("usercount = DESC").limit(3)[index].city
      @user_count = Billing.select("city, count(user_id) AS usercount").group(:city).order("usercount = DESC").limit(3)[index].city
